...
export default function ClassesPage() {
  ...
  const setFilterPanel = sidebar?.setFilterPanel;
  const clearFilter = sidebar?.clearFilter;
  useEffect(() => {
    if (!setFilterPanel) return;
    setFilterPanel(
      <ClassFilters
        filters={filters}
        onChange={setFilters}
    ...
    return () => clearFilter?.();
  }, [setFilterPanel, clearFilter, filters, classTypes]);
  ...
  const filtered = useMemo(() => {
    ...
    return classes.filter((cls) => {
      ...
      if (filters.typeSlug && cls.class_type?.slug !== filters.typeSlug)
        return false;
      return true;
    });
  }, [classes, filters]);
        ...
        <div
          className={
            sidebar
              ...
              : "grid lg:grid-cols-[280px_1fr] gap-8 items-start"
          }
        >
              ...
              {!isLoading && (
                <p className="text-sm text-charcoal-muted">
                  ...
                  {hasActiveFilters && (
                    <button
                      onClick={() =>
                        setFilters({ search: "", level: "", typeSlug: "" })
                      }
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-warm-gray-dark/30">
                <div className="w-14 h-14 rounded-2xl bg-warm-gray flex items-center justify-center mb-4">
                  <SearchX
                    className="w-6 h-6 text-charcoal-subtle"
                    aria-hidden="true"
                  />
                </div>
                ...
                <p className="text-charcoal-muted text-sm max-w-xs mb-5">
                  No encontramos clases con esos filtros. Intenta con otros
                  criterios.
                </p>
                <Button
                  variant="outline"
