    ...
    <>
      ...
      <main className="max-w-2xl mx-auto px-4 py-12 md:py-20 min-h-[calc(100vh-4rem)]">
        ...
        <div className="text-center mb-10 animate-fade-in-up">
          ...
          <p className="text-charcoal-muted text-lg">{t('confirmation.subtitle')}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-warm-gray-dark/40 shadow-card overflow-hidden mb-6 animate-fade-in">
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-charcoal-muted">{t('confirmation.orderNumber')}</span>
            ...
            {/* Email */}
            <div className="flex items-center justify-between">
              <span className="text-sm text-charcoal-muted">{t('confirmation.email')}</span>
              <span className="text-sm text-charcoal font-medium">{displayEmail}</span>
            </div>
              ...
              <div className="pt-3 border-t border-warm-gray-dark/50 space-y-2">
                {items.map((item, i) => (
                  <div key={i} className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-charcoal">{formatMXN(item.price)}</span>
                  </div>
                ))}
                <div className="flex justify-between pt-2 border-t border-warm-gray-dark/40 font-bold">
                  ...
                  <span className="text-sage text-lg font-display">{formatMXN(total)}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {intentType === 'booking' && (
          <div className="bg-sage-muted/40 rounded-xl p-4 flex items-start gap-3 mb-6 animate-fade-in">
            <Calendar className="w-5 h-5 text-sage shrink-0 mt-0.5" aria-hidden="true" />
            <div>
              <p className="text-sm font-semibold text-charcoal">Tu reserva está confirmada</p>
