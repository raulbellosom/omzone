      ...
      <CardContent className="p-5 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          {loading ? (
            ...
            <>
              <p className="font-bold text-charcoal font-display leading-tight truncate">
                {value}
              </p>
    ...
    >
        ...
        <section>
          ...
          {lBookings ? (
            ...
            <Card className="overflow-hidden">
              <div
                className="h-1 bg-linear-to-r from-sage to-olive"
                aria-hidden="true"
              />
              <CardContent className="p-5">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-charcoal-muted mb-4">
                  <span>
                    {format(
                      new Date(nextBooking.session.session_date),
                      "EEEE d 'de' MMMM",
                  ...
                  <span>
                    {nextBooking.session.class.duration_min} {t("bookings.min")}
                  </span>
                </div>
      ...
      <section className="mt-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            {
              label: t("common:actions.bookClass"),
              to: ROUTES.CLASSES,
              icon: CalendarCheck,
            },
            {
              label: t("common:actions.explorePlans"),
              to: ROUTES.PACKAGES,
              icon: Award,
            },
      ...
      <section className="mt-8">
        ...
        <Link to={ROUTES.WELLNESS}>
          <Card className="group hover:-translate-y-0.5 hover:shadow-card-hover transition-all duration-200 overflow-hidden">
            <div
              className="h-1 bg-linear-to-r from-olive-light to-sage-muted"
              aria-hidden="true"
            />
            <CardContent className="p-5 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-linear-to-br from-sage-muted to-olive-light flex items-center justify-center shrink-0">
                <GlassWater
                  className="w-5 h-5 text-sage-darker"
                  aria-hidden="true"
                />
              </div>
              <div className="flex-1">
                <p className="font-medium text-charcoal text-sm">
                  Smoothie Green Balance
                </p>
                <p className="text-xs text-charcoal-muted mt-0.5">
                  El complemento perfecto después de tus clases.
                </p>
              </div>
              <ArrowRight className="w-4 h-4 text-charcoal-subtle shrink-0 transition-transform group-hover:translate-x-0.5" />
            </CardContent>
