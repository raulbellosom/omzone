...
export default function CustomerProfilePage() {
  ...
  // Keep form in sync whenever the server-authoritative user object changes
  // (e.g. after a successful profile save triggers refreshUser).
  useEffect(() => {
    if (user) {
      setForm({
        first_name: user.first_name ?? "",
  ...
  async function handleAvatarChange(e) {
    ...
    try {
      ...
      const detail = err?.type || err?.message || String(err);
      toast.error(`${t("common:errors.generic")} (${detail})`);
    } finally {
  ...
  async function handleSavePassword(e) {
    ...
    const errs = {};
    if (!passForm.current)
      errs.current = t("validation:required", { ns: "validation" });
    if (!passForm.next || passForm.next.length < 8)
      errs.next = t("validation:minLength", { ns: "validation", min: 8 });
    if (passForm.next !== passForm.confirm)
      errs.confirm = t("validation:passwordMatch", { ns: "validation" });
