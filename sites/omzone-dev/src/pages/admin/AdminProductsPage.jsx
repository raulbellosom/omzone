...
import { useAdminProducts, useToggleProduct, useCreateProduct, useUpdateProduct } from '@/hooks/useAdmin'
...
const EMPTY_FORM = {
  ...
  product_type: 'snack', price: 0, is_addon_only: false, is_featured: false, enabled: true,
}
...
export default function AdminProductsPage() {
  ...
  function openEdit(p) {
    setEditing(p)
    setForm({ name_es: p.name_es ?? '', name_en: p.name_en ?? '', description_es: p.description_es ?? '', description_en: p.description_en ?? '', product_type: p.product_type ?? 'snack', price: p.price ?? 0, is_addon_only: p.is_addon_only ?? false, is_featured: p.is_featured ?? false, enabled: p.enabled ?? true })
    setDialogOpen(true)
  }
  function handleSubmit() {
    const mut = editing ? updateProduct : createProduct
    const payload = editing ? { productId: editing.$id, data: { ...form, price: Number(form.price) } } : { ...form, price: Number(form.price) }
    mut.mutate(payload, {
      onSuccess: () => { setDialogOpen(false); toast.success(editing ? 'Producto actualizado' : 'Producto creado') },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 animate-fade-in-up">
                ...
                >
                  <CardContent className="p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-charcoal-subtle hover:text-charcoal" onClick={() => openEdit(product)}>
      ...
      <AdminFormDialog
        ...
        title={editing ? t('products.editProduct', 'Editar Producto') : t('products.newProduct', 'Nuevo Producto')}
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            <Label>{t('products.fields.descriptionEs', 'Descripción ES')}</Label>
            <Input value={form.description_es} onChange={(e) => setForm(f => ({ ...f, description_es: e.target.value }))} />
          </div>
          <div className="space-y-1">
            <Label>{t('products.fields.descriptionEn', 'Descripción EN')}</Label>
          ...
          <div className="space-y-1">
            ...
            >
              {['smoothie','snack','supplement','plan','addon'].map(type => (
                <option key={type} value={type}>{t(`products.types.${type}`, type)}</option>
              ))}
            </select>
          </div>
          <div className="space-y-1">
            <Label>{t('products.fields.price', 'Precio')}</Label>
            <Input type="number" min="0" step="5" value={form.price} onChange={(e) => setForm(f => ({ ...f, price: e.target.value }))} />
          </div>
          <div className="col-span-2 flex items-center gap-4">
            <label className="flex items-center gap-2 text-sm text-charcoal cursor-pointer">
              <input type="checkbox" checked={form.is_addon_only} onChange={(e) => setForm(f => ({ ...f, is_addon_only: e.target.checked }))} className="accent-sage" />
