...
import { TrendingUp, CalendarCheck, Users, Award, ShoppingBag, ArrowRight, Zap } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Skeleton } from '@/components/ui/skeleton'
import { useAdminOverview, useAdminOrders, useAdminLeads, useAdminSessions } from '@/hooks/useAdmin'
  ...
  return (
    <Card>
      <CardContent className="p-5 flex items-center gap-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${colors[accent] ?? colors.sage}`}>
          <Icon className="w-5 h-5" />
        </div>
        <div className="min-w-0 flex-1">
          {loading
            ? <><Skeleton className="h-5 w-20 mb-1" /><Skeleton className="h-3.5 w-28" /></>
            : <>
                <p className="font-bold text-charcoal font-display leading-tight">{value}</p>
...
export default function AdminDashboardPage() {
  ...
  const upcomingSessions = sessions?.filter((s) => s.status !== 'cancelled').slice(0, 4) ?? []
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 animate-fade-in-up">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-charcoal text-sm">{t('dashboard.recentOrders')}</h2>
            <Link to={ROUTES.ADMIN_ORDERS} className="text-xs text-sage hover:underline font-medium flex items-center gap-1">
              ...
              <Card>
                <CardContent className="p-0">
                  <ul className="divide-y divide-warm-gray-dark/20">
                    {recentOrders.map((o) => (
                      <li key={o.$id} className="flex items-center justify-between px-5 py-3.5 gap-3">
                        <div className="min-w-0">
                          <p className="text-[10px] font-mono text-charcoal-subtle">{o.order_no}</p>
                          <p className="text-sm text-charcoal font-medium truncate">{o.customer_email}</p>
                        </div>
                        <div className="shrink-0 flex items-center gap-2">
                          <Badge variant={PAYMENT_COLORS[o.payment_status] ?? 'default'} className="text-[10px]">
                            {t(`orders.paymentStatus.${o.payment_status}`)}
                          </Badge>
                          <span className="font-semibold text-charcoal text-sm">{formatMXN(o.grand_total)}</span>
                        </div>
                      </li>
                    ))}
                  </ul>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-charcoal text-sm">{t('dashboard.recentLeads')}</h2>
            <Link to={ROUTES.ADMIN_LEADS} className="text-xs text-sage hover:underline font-medium flex items-center gap-1">
              {t('dashboard.viewAll')} <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          {lLeads
            ...
            : (
              <Card>
                <CardContent className="p-0">
                  <ul className="divide-y divide-warm-gray-dark/20">
                    {recentLeads.map((l) => (
                      <li key={l.$id} className="flex items-center justify-between px-5 py-3.5 gap-3">
                        <div className="min-w-0">
                          <p className="text-sm text-charcoal font-medium truncate">{l.full_name}</p>
                          <p className="text-[10px] text-charcoal-muted">{t(`leads.interest.${l.interest_type}`)}</p>
                        </div>
                        <Badge variant={STATUS_COLORS[l.status] ?? 'default'} className="text-[10px] shrink-0">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-charcoal text-sm">{t('dashboard.upcomingSessions')}</h2>
          <Link to={ROUTES.ADMIN_SESSIONS} className="text-xs text-sage hover:underline font-medium flex items-center gap-1">
            {t('dashboard.viewAll')} <ArrowRight className="w-3 h-3" />
          </Link>
        </div>
        {lSessions
                  ...
                  <Card key={s.$id}>
                    <CardContent className="p-4">
                      ...
                      <p className="text-[10px] text-charcoal-muted mb-3">
                        {format(new Date(s.session_date), "EEE d MMM · HH:mm", { locale: dateFnsLocale })}
                      </p>
