      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-charcoal font-medium truncate">{order.customer_email}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Badge variant={PAYMENT_BADGE[order.payment_status] ?? 'default'} className="text-[10px]">
            {t(`orders.paymentStatus.${order.payment_status}`)}
          </Badge>
          <Badge variant={FULFILL_BADGE[order.fulfillment_state] ?? 'default'} className="text-[10px]">
            {t(`orders.fulfillment.${order.fulfillment_state}`)}
          </Badge>
          <span className="font-semibold text-charcoal text-sm">{formatMXN(order.grand_total)}</span>
        </div>
      </button>
      {open && (
            ...
            return (
              <li key={item.$id} className="flex items-center justify-between gap-3 text-sm">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-charcoal truncate">{item.title_snapshot}</span>
                  <span className="text-charcoal-muted text-xs shrink-0">×{item.quantity}</span>
                </div>
                <span className="font-medium text-charcoal shrink-0">{formatMXN(item.line_total)}</span>
              </li>
            )
          })}
        </ul>
            ...
            <Card>
              <CardContent className="p-0">
                <ul className="divide-y divide-warm-gray-dark/20">
                  {orders.map((o) => (
                    <OrderRow key={o.$id} order={o} t={t} dateFnsLocale={dateFnsLocale} />
                  ))}
                </ul>
              </CardContent>
            </Card>
