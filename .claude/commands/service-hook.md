---
description: Crear servicios mock y hooks React Query para data fetching
---

# Service + Hook Layer — OMZONE

Crea el servicio mock y/o hook React Query descrito a continuación.
...
import { getEntities, getEntityBySlug } from "@/services/mocks/entityService.mock";
...
import { adminToggleEntity, adminUpsertEntity } from "@/services/mocks/entityService.mock";
...
export async function getEntities({ featuredOnly } = {}) {
  ...
  const { documents } = await databases.listDocuments(DB_ID, "entities", queries);
  return documents;
}
```
