...
export default function ClassDetailPage() {
  ...
  const { data: sessions, isLoading: loadingSessions } = useSessionsByClass(cls?.$id)
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-8 items-start">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {spotsLeft !== null && (
                    <span className={cn(
                      ...
                      spotsLeft < 4 ? 'text-amber-600' : 'text-charcoal-muted'
                    )}>
              ...
              <p className="text-2xl font-bold text-sage font-display shrink-0">
                {formatMXN(price)}
              </p>
            </div>
            ...
            <section aria-labelledby="about-class-heading" className="mb-8">
              <h2 id="about-class-heading" className="text-lg font-semibold text-charcoal mb-3">
                {t('detail.about')}
              </h2>
            ...
            {/* Instructor */}
            {cls.instructor && (
              <section aria-labelledby="instructor-heading" className="bg-warm-gray rounded-2xl p-5 mb-8">
                <h2 id="instructor-heading" className="text-sm font-semibold text-charcoal mb-3">
                  {t('detail.instructor')}
                </h2>
            ...
            <section aria-labelledby="sessions-heading" className="mb-8">
              <h2 id="sessions-heading" className="text-lg font-semibold text-charcoal mb-4">
                {t('detail.schedule')}
              </h2>
              {loadingSessions ? (
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <span className="text-sm font-medium text-charcoal flex items-center gap-2">
                            <Calendar className="w-4 h-4 text-sage shrink-0" aria-hidden="true" />
                        ...
                        <div className="flex flex-col items-end gap-1">
                          {full ? (
                            <Badge variant="warm">{t('detail.sessionFull')}</Badge>
                          ) : (
                            <>
                              <span className={cn(
                                'text-xs',
                                spots < 4 ? 'text-amber-600 font-medium' : 'text-charcoal-muted'
                              )}>
                                {t('card.spots', { count: spots })}
                              </span>
                              {active && (
                                <span className="text-xs font-semibold text-sage">
                                  {formatMXN(sess.price_override ?? cls.base_price)}
                                </span>
                              )}
                            </>
                          )}
                ...
                <p className="text-charcoal-muted text-sm">{t('detail.noSessions')}</p>
              )}
            </section>
            ...
            {extras?.length > 0 && (
              <section aria-labelledby="extras-heading" className="mb-8">
                <div className="mb-4">
                  <h2 id="extras-heading" className="text-lg font-semibold text-charcoal">
                    {t('detail.extras')}
                  </h2>
                  <p className="text-sm text-charcoal-muted">{t('detail.extrasDesc')}</p>
                </div>
                  ...
                  {extras.slice(0, 4).map((product) => {
                    ...
                    return (
                      <div
                        ...
                        tabIndex={0}
                        onKeyDown={(e) => e.key === ' ' && toggleExtra(product.$id)}
                      >
                        {isSelected && (
          ...
          <aside className="lg:sticky lg:top-24">
            <div className="bg-white rounded-2xl border border-warm-gray-dark/50 shadow-card p-5">
              ...
              {activeSession && (
                <div className="bg-cream rounded-xl p-3 mb-4 text-sm">
                  ...
                  <p className="text-charcoal-muted text-xs mt-1 ml-5">
                    {activeSession.location_label} · {formatDuration(cls.duration_min)}
                  </p>
                </div>
              )}
                ...
                {selectedExtras.length > 0 && (
                  <div className="flex justify-between">
                    <span className="text-charcoal-muted">
                      {t('detail.extrasCount', { count: selectedExtras.length })}
                    </span>
                    <span className="font-medium">{formatMXN(extrasTotal)}</span>
                  </div>
                )}
                <div className="flex justify-between pt-2 border-t border-warm-gray-dark/40 text-base font-bold text-charcoal">
                  <span>{t('detail.total')}</span>
                  <span className="text-sage">{formatMXN(price + extrasTotal)}</span>
                </div>
              </div>
              ...
              >
                <Link
                  to={activeSession ? ROUTES.BOOKING(activeSession.$id) : '#'}
